...
Machine contract & The Kria implementation generates the authoritative, hardware-neutral \texttt{urn:fnal:gizmo} OPC~UA model; the companion GIZMo--SC ICD documents it for human review. Any other \GIZMO{} implementation shall conform to it. Current released model baseline: 1.3.1. \\
...
contract. Its namespace URI is \nolinkurl{urn:fnal:gizmo}; the GIZMo--SC ICD
documents that machine contract for human review. Any other \GIZMO{} hardware
implementation shall conform to it. Site
endpoints, certificates, trust lists, roles, and operator authorization remain
controlled configuration. A ZedBoard server is under development as an
independent conforming producer. Slow Controls may prepare against the common
contract, but shall not treat the ZedBoard connection as commissioned until its
contract, identity, non-interference, and measurement-acceptance gates pass.
...
The GIZMo--SC ICD defines the canonical OPC~UA namespace, types, meanings,
quality and timestamp rules, configuration writes, and operations. The public
Kria runtime implements the present baseline and also defines its own component
services, historian, and dashboard. It intentionally does not publish production
...
The planned ZedBoard OPC~UA server shall conform to the Kria-authoritative
\texttt{urn:fnal:gizmo} contract, preserve a distinct endpoint and device
identity, and publish explicit non-good quality for unsupported or stale
values. It shall operate as a non-owning observational sidecar: start, stop,
upgrade, disconnect, load, or failure of the server must not change either the
Kria system or the preserved ZedBoard controller, display, stimulus, relay, or
physical alarm behavior. The server remains under development. Ignition
history and alarms shall not be enabled until schema, identity, coherent-cycle,
display-comparison, restart, network-loss, and non-interference tests pass. The
development profile permits only the separately reviewed 0--1023-ohm
\texttt{Configuration.ThresholdOhm} subset; production use of that write
remains gated, and every other ZedBoard write and method is unsupported.
...
\begin{enumerate}
  ...
  \item Public GIZMo Kria runtime, OPC~UA reference implementation, historian,
  and dashboard documentation; the companion ICD remains the machine-interface
  authority.\\
